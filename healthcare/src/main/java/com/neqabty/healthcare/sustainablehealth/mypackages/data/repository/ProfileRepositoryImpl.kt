...
class ProfileRepositoryImpl @Inject constructor(private val profileDS: ProfileDS) : ProfileRepository {
    ...
    override fun deleteFollower(followerId: Int, subscriberId: String, mobile: String): Flow<Boolean> {
        return flow {
            emit(profileDS.deleteFollower(DeleteFollowerBody(followerId, subscriberId, mobile)))
        }
    }
