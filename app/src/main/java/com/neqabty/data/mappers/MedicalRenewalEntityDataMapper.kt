...
class MedicalRenewalEntityDataMapper @Inject constructor() : Mapper<MedicalRenewalEntity, MedicalRenewalData>() {
    ...
    override fun mapFrom(from: MedicalRenewalEntity): MedicalRenewalData {
        return MedicalRenewalData(
                ...
                contact = from.contact?.let { return@let MedicalRenewalData.ContactData(name = it.name, syndicateName = it.syndicateName, isNew = it.isNew) },
                followers = from.followers?.map { return@map MedicalRenewalData.FollowerItem(name = it.name, id = it.id, isDeleted = it.isDeleted, birthDate = it.birthDate, pic = it.pic, attachments = it.attachments) }
        )
    }
}