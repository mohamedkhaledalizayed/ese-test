...
class MedicalRenewalEntityDataMapper @Inject constructor() : Mapper<MedicalRenewalEntity, MedicalRenewalData>() {
    ...
    override fun mapFrom(from: MedicalRenewalEntity): MedicalRenewalData {
        return MedicalRenewalData(
                ...
                followers = from.followers?.map { return@map MedicalRenewalData.FollowerItem(name = it.name, name1 = it.name1, name2 = it.name2, name3 = it.name3, name4 = it.name4, id = it.id, isDeleted = it.isDeleted, modificationReason = it.modificationReason, birthDate = it.birthDate, pic = it.pic, attachments = it.attachments, mobile = it.mobile, nationalId = it.nationalId, relationType = it.relationType, relationTypeName = it.relationTypeName, gender = it.gender) }
        )
    }
}